...
const Header: React.FC<ChildProps> = (props) => {
    ...
    return (
        <div className={props.email ? "bg-stone-100 absolute top-0 left-0 w-screen z-20 pt-10 pb-2 md:px-20 items-center justify-between font-['Helvetica'] text-sm flex flex-row-reverse" :
            ...
            {props.email && <>
                <div className={"flex flex-col space-y-1 h-14 items-center justify-end"}>
                    <button className={"p-1 items-center justify-center " +
                        "font-semibold text-stone-600 hover:-translate-y-0.5"}
                        onClick={() => signOut()}>
                        Sign Out
                    </button>
                </div>
                <div className=" flex space-x-2 items-end text-lg font-bold h-14">
                    <Image
                        className="rounded-xl"
                    ...
                    <span>Workflows &gt; demo workflow</span>
                </div>
            </>}
        </div>
    );
